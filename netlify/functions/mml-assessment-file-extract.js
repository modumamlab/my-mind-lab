  ...
  properties:{
    ...
    tciScores:{
      type:'ARRAY',items:{
        type:'OBJECT',
        properties:{
          code:{type:'STRING'},scale:{type:'STRING'},rawScore:{type:'NUMBER'},tScore:{type:'NUMBER'},percentile:{type:'NUMBER'},source:{type:'STRING'}
        },
        required:['code','scale','rawScore','tScore','percentile','source']
      }
    },
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(extracted.tciScores)){
        ...
        for(const row of extracted.tciScores){
          const code=String(row?.code||'').trim().toUpperCase();
          const rawScore=Number(row?.rawScore),tScore=Number(row?.tScore),percentile=Number(row?.percentile);
          if(!allowed.has(code)||!Number.isFinite(rawScore)||!Number.isFinite(tScore)||!Number.isFinite(percentile)||percentile<0||percentile>100)continue;
          if(!byCode.has(code))byCode.set(code,{code,scale:clean(row?.scale,80),rawScore,tScore,percentile,level:percentile<=30?'낮음':percentile>=70?'높음':'보통',source:clean(row?.source,160)});
        }
