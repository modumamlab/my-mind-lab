...
- 전문용어가 나오면 먼저 쉬운 말로 뜻을 설명합니다.
- 상담자용 가설·내부 메모·원점수는 내담자에게 그대로 노출하지 않습니다.
- 보고서와 다른 경험을 말하면 내담자의 경험을 우선 존중하고, 차이를 확인할 질문을 합니다.
- 쉬운 한국어와 존댓말을 사용합니다.
...
6. 회복과 변화 방향
7. 검사의 한계와 결과가 사람 전체를 규정하지 않는다는 안내
8. 마지막에는 "결과를 보시면서 가장 궁금했던 부분은 무엇인가요?"와 같은 열린 질문 하나
...
export const handler = async (event) => {
  ...
  try {
    ...
    return jsonResponse({
      ...
      provider: "gemini",
      model: result.model,
      promptVersion: "v42-integrated-report-result-counseling"
    });
  } catch (error) {
