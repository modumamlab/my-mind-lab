...
function buildPrompt(body){
  ...
  const audience=body.testType==='PAT'?'부모가 자신의 양육태도를 이해하고 실제 양육에 적용할 수 있도록':'부모가 아동의 타고난 기질을 이해하고 양육과 생활지도에 적용할 수 있도록';
...
핵심 목표:
- 척도별 코드와 점수를 순서대로 나열하지 않습니다.
- 여러 척도가 서로 어떻게 영향을 주는지 통합해 설명합니다.
- 검사 결과가 일상, 관계, 감정조절, 양육 장면에서 어떻게 나타날 수 있는지 쉬운 말로 설명합니다.
- 점수 자체보다 점수 조합이 만들어내는 패턴을 중심으로 작성합니다.
...
- PAT는 지지표현·합리적 설명·성취압력·간섭·처벌·감독·과잉기대·비일관성이 함께 나타날 때의 양육 경험을 부모 비난 없이 설명합니다.
- 점수가 미입력된 경우 수치 해석은 하지 않고 수준만 반영합니다.
- 같은 척도명과 점수를 여러 항목에서 반복하지 않습니다.
- 결과는 AI 초안이며 임상심리사의 검토가 필요합니다.

좋지 않은 예:
- 활동성 높음, 조심성 낮음, 긍정정서 높음.

좋은 예:
- 활동량이 많고 새로운 상황에 빠르게 참여하는 편이지만, 위험을 충분히 살피기 전에 행동할 수 있습니다. 따라서 제지보다 미리 선택지를 알려주고 멈춰 생각할 시간을 주는 방식이 더 잘 맞을 수 있습니다.

아래 JSON만 반환하세요.
{
  "oneLine":"결과의 핵심 패턴을 따뜻하고 정확하게 한 문장으로 요약",
  "overall":"척도들의 상호작용을 중심으로 정서, 행동, 관계, 자기조절의 흐름을 4~7문단으로 설명. 척도 코드 나열 금지",
  "strength":"일상에서 활용 가능한 강점과 보호요인 3~6개",
  "caution":"특정 상황에서 어려울 수 있는 점과 과도한 해석을 피하기 위한 주의사항 3~6개",
  "coaching":"가정과 상담에서 바로 적용할 수 있는 구체적인 제안 4~7개",
  "scaleInterpretations":"척도별 설명은 짧게 유지하고, 각 척도가 다른 척도와 결합될 때의 의미를 포함해 작성"
}`;
}
...
export const handler=async(event)=>{
  ...
  try{
    ...
    return jsonResponse({interpretation,model:result.model,promptVersion:'sts-pat-v2-integrated-narrative'});
