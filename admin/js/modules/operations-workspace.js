...
function reservationView(){
  return layout(`${reservationSyncStatus()}<div class="space-y-3">
    ...
    ${state.reservations.map(r=>{
      ...
      const methodLabel=(scheduleMethod==='AI상담'||String(r.consultationMethod||'')==='AI상담')
        ? 'AI상담'
        : (isAppApplication&&scheduleMethod==='온라인 심리검사'
          ? '온라인 심리검사'
      ...
      return `<section class="rounded-[1.6rem] border border-slate-100 bg-white p-4 shadow-sm sm:p-5">
        ...
        <div class="mt-3 grid grid-cols-2 gap-2 lg:grid-cols-5">
          <div class="rounded-xl bg-slate-50 px-3 py-2.5">
            <p class="text-[10px] font-bold text-slate-400">접수일</p>
            <p class="mt-1 truncate text-xs font-extrabold text-slate-800">${esc(String(r.applicationDate||r.createdAt||'').slice(0,10)||'-')}</p>
          </div>
          <div class="rounded-xl bg-slate-50 px-3 py-2.5">
            <p class="text-[10px] font-bold text-slate-400">상담일정</p>
...
function deleteManualClient(key){
  ...
  if(!c){alert('홈페이지 예약에서 생성된 내담자는 예약관리와 심리평가센터에서 연결 기록을 관리해 주세요.');return}
...
function clientManagementView(){
  ...
  return layout(`<div class="space-y-6">
    <section class="rounded-[2rem] bg-slate-950 p-6 text-white sm:p-8"><p class="text-xs font-extrabold text-emerald-300">CLIENT MANAGEMENT</p><h2 class="mt-2 text-2xl font-extrabold">내담자관리</h2><p class="mt-2 text-sm text-slate-300">홈페이지 예약 여부와 관계없이 전화·방문·검사 의뢰 내담자를 직접 등록하고 관리합니다.</p></section>
